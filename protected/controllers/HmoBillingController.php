...
class HmoBillingController extends RController
{
    ...
    public function actionPrintBilling($to_excel = false)
    {
        if($_GET){
            ...
            $print = str_replace("[dasma_address]",$settings->dasma_address,$print);
    ...
    /*public function actionPrintbillingcategory($to_excel = false)
    {
        if($_GET){
            ...
            if($apeT != 0) { $apeT = number_format($apeT,2); }else{ $apeT = ""; }
            if($medT != 0) { $medT = number_format($medT,2); }else{ $medT = ""; }
            if($radT != 0) { $radT = number_format($radT,2); }else{ $radT = ""; }
            if($labT != 0) { $labT = number_format($labT,2); }else{ $labT = ""; }
            if($ptT != 0) { $ptT = number_format($ptT,2); }else{ $ptT = ""; }
            if($docT != 0) { $docT = number_format($docT,2); }else{ $docT = ""; }
            if($othT != 0) { $othT = number_format($othT,2); }else{ $othT = ""; }
    ...
    public function actionPrintBillingCategoryOld($to_excel = false)
    {
        if($_GET){
            $connection=Yii::app()->db;
            $billing_id =  $_GET["id"];
            ...
            $hmo_add = $hmo->street1."<br/>".$hmo->street2.", ".$hmo->barangay."<br/>".$hmo->city.", ".$hmo->province;
            $url = Yii::app()->getBasePath() ;
            $print = implode("", file(Yii::app()->getBasePath().'/html/hmobillingcategory.html'));
            $logo = 'http://'.$_SERVER["HTTP_HOST"].'/images/printdiagresult/wpprintlogo.png';
            $print = str_replace("[logopath]",$logo,$print);
            $print = str_replace("[HMO]",$hmo->name,$print);
            ...
            //SOA No.
            $print = str_replace("[soa_no]","WPR-".$billing_id,$print);
            ...
            foreach($dataReader as $row) {
                ...
                $qryhi = "select * from hmo_form_items_category where hmo_form_item_id = ".$row["itemid"];
                $cmdhi = $connection->createCommand($qryhi);
                $drhhi = $cmdhi->query();

                $Xape = ""; $Xmed = ""; $Xrad = ""; $Xlab = ""; $Xpt = ""; $Xspa = ""; $Xothers = 0; $Xotherstotal = 0;
                foreach($drhhi as $rwhi) {
                    switch($rwhi["category"]){
                        case 'Rehabilitation Medicine And Physical Therapy':$Xpt = number_format($rwhi["amount"],2);break;
                        case 'Annual Physical Exam':$Xape = number_format($rwhi["amount"],2);break;
                        case 'Annual Package B':$Xape = number_format($rwhi["amount"],2);break;
                        case 'Annual Package C':$Xape = number_format($rwhi["amount"],2);break;
                        case 'Annual Package D':$Xape = number_format($rwhi["amount"],2);break;
                        case 'Radiology and Ancillary':$Xrad = number_format($rwhi["amount"],2);break;
                        case 'Laboratory':$Xlab = number_format($rwhi["amount"],2);break;
                        case 'Aesthetics':$Xspa = number_format($rwhi["amount"],2);break;
                        case 'Medical':$Xmed = number_format($rwhi["amount"],2);break;
                        default: $Xothers = $Xothers + $rwhi["amount"]; break;
                    }
                    $Xotherstotal = number_format($Xothers,2);
                }

                if($row["payto"] == "WPCLINIC"){
                    $print = str_replace("[ape]","",$print);
                    $print = str_replace("[med]","",$print);
                    $print = str_replace("[rad]","",$print);
                    $print = str_replace("[lab]","",$print);
                    $print = str_replace("[pt]","",$print);
                    $print = str_replace("[spa]","",$print);
                    $print = str_replace("[oth]","",$print);

                    $holder .= "<td class='money'>".$Xape."</td>";
                    $holder .= "<td class='money'>".$Xmed."</td>";
                    $holder .= "<td class='money'>".$Xrad."</td>";
                    $holder .= "<td class='money'>".$Xlab."</td>";
                    $holder .= "<td class='money'>".$Xpt."</td>";
                    $holder .= "<td class='money'>".$Xspa."</td>";
                    $Xotherstotal = $Xotherstotal > 0 ? $Xotherstotal : "" ;
                    $holder .= "<td class='money'>".$Xotherstotal."</td>";
                }else{
                    $holder .= "<td class='money'></td>";
                    $holder .= "<td class='money'></td>";
                    $holder .= "<td class='money'></td>";
            ...
            //compute clinic charge total
            $arrayCategory = null;
            $arrayCategory[] = 'Annual Physical Exam';
            $arrayCategory[] = 'Aesthetics';
            $arrayCategory[] = 'Laboratory';
            $arrayCategory[] = 'Medical';
            $arrayCategory[] = 'Others';
            $arrayCategory[] = 'Radiology and Ancillary';
            $arrayCategory[] = 'Rehabilitation Medicine And Physical Therapy';

            foreach($arrayCategory as $value) {
                $query = "select sum(amount) as 
                    sum from hmo_form_items_category where hmo_billing_id = 
                    $billing_id AND category = '$value' AND hmo_form_item_id IN (78682,78680)";
                $command=$connection->createCommand($query);
                $dataReader=$command->query();
                foreach($dataReader as $row) {
                    $category_sum = number_format($row["sum"],2);
                }
                $print = str_replace("[$value]",$category_sum,$print);
            }
            ...
            //compute doctors procedure total
            $query = "select sum(b.charge_fee) as doc_proc_sum1
                        from hmo_form a
                        left join hmo_form_items b
                        on a.id = b.hmo_form_id
                        where a.hmo_billing_id = $billing_id
            ...
            foreach($dataReader as $row) {
                $doc_proc_sum1 = number_format($row["doc_proc_sum1"],2);
                $dc1 = $row["doc_proc_sum1"];
            }

            //compute doctors professional fee total
            $query = "select sum(b.charge_fee) as doc_proc_sum2
                        from hmo_form a
                        left join hmo_form_items b
                        on a.id = b.hmo_form_id
                        where a.hmo_billing_id = $billing_id
            ...
            foreach($dataReader as $row) {
                $doc_proc_sum2 = number_format($row["doc_proc_sum2"],2);
                $dc2 = $row["doc_proc_sum2"];
            }

            $print = str_replace("[dp]",$doc_proc_sum1,$print);
            $print = str_replace("[pf]",$doc_proc_sum2,$print);

            $doctors_total = (int)$dc1 + (int)$dc2;
            $print = str_replace("[dt]",number_format($doctors_total,2),$print);
            ...
            */
            ...
            if ($to_excel == true){
                $filename = "HMO_Billing_With_Category_$billing_id.xls";
                header("Content-Disposition: attachment; filename=\"$filename\"");
                header("Content-Type: application/vnd.ms-excel");
                echo $print;
            }else{
                echo "&nbsp;&nbsp;<button class='noprint' onclick=\"window.location = '../exporttoexcelcategory/$billing_id'\" value='' >Excel</button>";
                echo $print;
            }
