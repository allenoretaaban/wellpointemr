...
class HmiController extends Controller
{
    ...
    private function printSoaSummary($hmo_bill_id)
    {
           ...
           $print = implode("", file(Yii::app()->getBasePath().'\modules\Hmi\html\printSummary.html')); 
    ...
    {
            $connection=Yii::app()->db;   
            $print = implode("", file(Yii::app()->getBasePath().'\modules\Hmi\html\printApe.html'));            
    ...
    {
         $connection=Yii::app()->db;   
            $print = implode("", file(Yii::app()->getBasePath().'\modules\Hmi\html\printDoctorPayable.html'));            
             
            $billing_id =  $hmo_bill_id;
            $billing = HmoBilling::model()->findByPk((int)$billing_id );             
			$print = str_replace("[soa_no]",$billing_id,$print);                        					
            $print = str_replace("[date_prepared]",date("Y-M-d",strtotime($billing->date_prepared) ), $print);                        
            $print = str_replace("[due_date]",date("Y-M-d",strtotime($billing->date_due) ),$print);                                    
            ...
            $qp1 = "select id from hmo_form where hmo_billing_id = $hmo_bill_id";
            $qp1Commandins = $connection->createCommand($qp1);
            $qp1DataReader = $qp1Commandins->query();
            foreach($qp1DataReader as $qp1RowIns) { 
                if($qp1RowIns["id"] != null) {
                    $qp1Arr[] =  $qp1RowIns["id"];
                }
            }
            $qp1StringIns = implode(",",$qp1Arr);

            // get INS
            $queryins = "select distinct a.claim_doctor_id from hmo_form_items a
                    left join hmo_form b on a.hmo_form_id = b.id where a.hmo_form_id in 
                    ($qp1StringIns)
                    and a.payto = 'DOCTOR' and a.service_type != 'APE' order by b.avail_date asc";
            $commandins = $connection->createCommand($queryins);
            $dataReaderIns = $commandins->query();
            foreach($dataReaderIns as $rowIns) { 
                if($rowIns["claim_doctor_id"] != null) {
                    $claim_doctor_id[] =  $rowIns["claim_doctor_id"];
                }
            }
            $claim_doctor_id_string = implode(",",$claim_doctor_id);
            
            //get the doctors first
            $query = "select x.id,x.firstname,x.lastname
                    from doctor x
                    where x.id in
                    ($claim_doctor_id_string) order by x.lastname asc";               
            $doctors = array();
            $command=$connection->createCommand($query);
            $doc_dataReader=$command->query();
            foreach($doc_dataReader as $row2) { 
                ...
                //count doc trnx rows count get ins
                $queryins = "select id from hmo_form where hmo_billing_id = $hmo_bill_id";
                $commandins = $connection->createCommand($queryins);
                $dataReaderIns = $commandins->query();
                foreach($dataReaderIns as $rowIns) { 
                    $hmo_form_id_ins[] = $rowIns["id"];
                }
                $hmo_form_id_ins_string = implode(",",$hmo_form_id_ins);
                ...
                $query = "select count(a.itemid) as rowscount,
                        ...
                        left join hmo_form b
                        on a.hmo_form_id = b.id
                        where a.hmo_form_id in ($hmo_form_id_ins_string)
                        and a.payto = 'DOCTOR'
                        and a.service_type != 'APE'
                        and a.claim_doctor_id = ".$row2["id"]."
                        order by b.avail_date asc";
                ...
                $qp2 = "select id from hmo_form where hmo_billing_id = $hmo_bill_id";
                $qp2Commandins = $connection->createCommand($qp2);
                $qp2DataReader = $qp2Commandins->query();
                foreach($qp2DataReader as $qp2RowIns) { 
                    if($qp2RowIns["id"] != null) {
                        $qp2Arr[] =  $qp2RowIns["id"];
                    }
                }
                $qp2StringIns = implode(",",$qp2Arr);
            ...
            if ($excel == true){
                $filename = "Hmi_PayToDoctor_$hmo_bill_id";
    ...
    private function printWpPayable($hmo_bill_id, $excel = false){
            $connection=Yii::app()->db;   
            $print = implode("", file(Yii::app()->getBasePath().'\modules\Hmi\html\printWpPayable.html'));            
            ...
            if ($excel == true){
                    $filename = "Hmi_PayToWp_$hmo_bill_id";
                    header("Content-Disposition: attachment; filename=\"$filename\""); 
                    header("Content-Type: application/vnd.ms-excel");
                    echo $print;
